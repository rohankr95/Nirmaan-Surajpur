@php $agreement = $work->agreement; @endphp
<form action="{{ route('work-agreement') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group row">
        <div class="col-sm-4">
            ...
            <input class="form-control" type="date" id="agreement_date" name="agreement_date"
                value="{{ optional($agreement)->agreement_date }}">
        </div>
        <div class="col-sm-4">
            ...
            <input class="form-control" type="text" id="work_order_no" name="work_order_no" placeholder="कार्य आदेश क्रमांक"
                value="{{ optional($agreement)->work_order_no }}">
        </div>
        <div class="col-sm-4">
            ...
            <input class="form-control" type="date" id="work_order_date" name="work_order_date"
                value="{{ optional($agreement)->work_order_date }}">
        </div>
    </div>
    ...
    <div class="form-group row">
        <div class="col-sm-4">
            ...
            <input class="form-control" type="number" step="0.01" min="0" id="work_order_amount" name="work_order_amount" placeholder="राशि"
                value="{{ optional($agreement)->work_order_amount }}">
        </div>
        <div class="col-sm-4">
            ...
            <select name="contractor_id" id="contractor_id" class="form-control form-select">
                ...
                @foreach(get_contractors() as $contractor)
                    <option value="{{ $contractor->contractor_id }}" {{ optional($agreement)->contractor_id == $contractor->contractor_id ? 'selected' : '' }}>{{ $contractor->contractor_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-sm-4">
            ...
            @if(optional($agreement)->upload_file)
                <small><a href="{{ asset($agreement->upload_file) }}" target="_blank">मौजूदा फाइल देखें</a></small>
            @endif
        </div>
    </div>
    ...
    <div class="form-group row">
        <div class="col-sm-12">
            ...
            <textarea name="remark" id="remark" cols="20" rows="1" class="form-control">{{ optional($agreement)->remark }}</textarea>
        </div>
    </div>
